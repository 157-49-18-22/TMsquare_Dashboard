...
function FormRegistrationLogs() {
  ...
  const fetchRegistrationLogs = async () => {
    try {
      ...
      // Filter for logs where formData.apiSuccess is true and process them
      ...
      for (const docSnapshot of snapshot.docs) {
        ...
        // Check if formData.apiSuccess is true (check different possible paths)
      ...
      setLogs(finalLogs);
      console.log(`Found ${finalLogs.length} logs with successful registration`);
  ...
  const handleExportToExcel = () => {
    ...
    exportToExcel(formattedData, 'FasTag_Registration_Logs', 'Registrations');
  ...
  return (
    <Box>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
        Showing successful FasTag registrations from form logs
      </Typography>
        ...
        {loading ? (
          ...
          <DataGrid
            ...
            components={{
              NoRowsOverlay: () => (
                <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%' }}>
                  <Typography>No registration logs found</Typography>
                </Box>
              )
            }}
          />
...
export default FormRegistrationLogs; 